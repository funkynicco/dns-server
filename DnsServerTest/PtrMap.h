...
{
	LPPTR_MAP lpMap = (LPPTR_MAP)malloc(sizeof(LPPTR_MAP_ENTRY_BLOCK) + sizeof(LPPTR_MAP_ENTRY) + sizeof(DWORD) + sizeof(DWORD) + sizeof(PTR_MAP_ENTRY) * dwBucketSize);
...
static int PtrMapGetBucketIndex(LPPTR_MAP lpMap, DWORD dwKey)
{
	...
	const unsigned char* data = &dwKey;
	...
	return result & (lpMap->dwBucketSize - 1);
}

static void* PtrMapFind(LPPTR_MAP lpMap, DWORD dwKey)
{
	LPPTR_MAP_ENTRY ptr = lpMap->Table[dwKey % lpMap->dwBucketSize];
...
static void PtrMapInsert(LPPTR_MAP lpMap, DWORD dwKey, void* lpValue)
{
	...
	DWORD dwIndex = dwKey % lpMap->dwBucketSize;
